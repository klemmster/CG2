...
Cube::Cube(VertexPtr vrtx0, VertexPtr vrtx1, VertexPtr vrtx2, VertexPtr vrtx3,
            VertexPtr vrtx4, VertexPtr vrtx5, VertexPtr vrtx6, VertexPtr vrtx7)
{
    Edge edge0(vrtx0, vrtx1);
    Edge edge1(vrtx1, vrtx2);
    Edge edge2(vrtx2, vrtx3);
    Edge edge3(vrtx3, vrtx0);
    Edge edge4(vrtx4, vrtx5);
    Edge edge5(vrtx5, vrtx6);
    Edge edge6(vrtx6, vrtx7);
    Edge edge7(vrtx7, vrtx4);
    Edge edge8(vrtx0, vrtx4);
    Edge edge9(vrtx1, vrtx5);
    Edge edge10(vrtx2, vrtx6);
    Edge edge11(vrtx3, vrtx7);
    ...
    m_Edges.push_back(edge7);
    m_Edges.push_back(edge8);
    m_Edges.push_back(edge9);
    m_Edges.push_back(edge10);
    m_Edges.push_back(edge11);

    m_vertices.push_back(vrtx0);
    m_vertices.push_back(vrtx1);
    m_vertices.push_back(vrtx2);
    m_vertices.push_back(vrtx3);
    m_vertices.push_back(vrtx4);
    m_vertices.push_back(vrtx5);
    m_vertices.push_back(vrtx6);
    m_vertices.push_back(vrtx7);
};
...
int Cube::getIndex(){
    int cubeIndex = 0;
    if( m_vertices.at(0)->getFunValue() <= 0) cubeIndex |= 1;
    if( m_vertices.at(1)->getFunValue() <= 0) cubeIndex |= 2;
    if( m_vertices.at(2)->getFunValue() <= 0) cubeIndex |= 4;
    if( m_vertices.at(3)->getFunValue() <= 0) cubeIndex |= 8;
    if( m_vertices.at(4)->getFunValue() <= 0) cubeIndex |= 16;
    if( m_vertices.at(5)->getFunValue() <= 0) cubeIndex |= 32;
    if( m_vertices.at(6)->getFunValue() <= 0) cubeIndex |= 64;
    if( m_vertices.at(7)->getFunValue() <= 0) cubeIndex |= 128;
    return cubeIndex;
}

VertexList Cube::getTriangles(){
    int index = getIndex();
    VertexList vertices;
    for(int i=0; i<13; i+=3){
        if( aCases[index][i] == -1)
            break;
        Edge edge1 = m_Edges.at((int)aCases[index][i]);
        Edge edge2 = m_Edges.at((int)aCases[index][i+1]);
        Edge edge3 = m_Edges.at((int)aCases[index][i+2]);

        vertices.push_back(getInterpolatedVertex(edge1));
        vertices.push_back(getInterpolatedVertex(edge2));
        vertices.push_back(getInterpolatedVertex(edge3));
    }
    return vertices;
}
...
MarchingCubes::MarchingCubes(const Grid3D& grid, const size_t dimX, const size_t dimY,
            const size_t dimZ):
    m_Grid(grid),
    m_dimX(dimX),
    m_dimY(dimY),
    ...
    for(int x=0; x<m_dimX-1; ++x){
        for(int y=0; y<m_dimY-1; ++y){
            for(int z=0; z<m_dimZ; ++z){
                ...
                VertexPtr vrtx2 = m_Grid.getVertex(x+1, y, z+1);
                VertexPtr vrtx3 = m_Grid.getVertex(x, y, z+1);
                VertexPtr vrtx4 = m_Grid.getVertex(x, y+1, z);
                VertexPtr vrtx5 = m_Grid.getVertex(x+1, y+1, z);
                VertexPtr vrtx6 = m_Grid.getVertex(x+1, y+1, z+1);
                VertexPtr vrtx7 = m_Grid.getVertex(x, y+1, z+1);
                Cube cube(vrtx0, vrtx1, vrtx2, vrtx3, vrtx4, vrtx5, vrtx6, vrtx7);
                VertexList triangleVerts = cube.getTriangles();
                for(auto elem : triangleVerts){
                    m_triangles.push_back(elem);
                }
            }
        }
    }
