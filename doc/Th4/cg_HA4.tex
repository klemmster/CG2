...
Ein Polyeder besteht aus planaren Primitiven. Auf diesen lassen sich diskrete Differentialoperationen ausführen. Polyeder sind außerdem geschlossen. Diese Eigenschaft ist wichtig, da an jedem Punkt klar definiert sein muss, ob dieser innerhalb oder außerhalb der Fläche ist. Weiterhin sind Polyeder manifold, wodurch sie sich effizient verwalten lassen, btw. viele Algorithmen nur auf manifold Surfaces funktionieren.

\task{2}

Es kann keine Kante aus einem Dreiecksnetz entfernt werden, wenn dieses dadurch non-manifold wird. Dies passiert beispielsweise an Rand-Edges:
\image{0.25}{res/edgeDeletionFail1.png}
Eine Löschung der oberen rechten Kante macht das Mesh non-manifold:
\image{0.25}{res/edgeDeletionFail2.png}
Die Löschung hat dazu geführt, dass ein Vertex Mitglied in zwei Triangle-Fans ist.
