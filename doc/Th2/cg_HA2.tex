...
\\
Der Algorithmus von Casteljau funktioniert über die folgende Rekursionsformel:\\\\
\eqs{$
...
D.h. das letzte Segment und die Ableitung unterscheiden sich nur in dem Faktor $n$, womit diese linear abhängig sind. Somit ist das letzte Segment eine Tangente der Kurve im ausgewerteten Punkt.
...
$}$ $\\\\
Dies lässt sich umstellen nach $c_{00}$:
\formel{$c_{10} x + c_{01}\cdot y + z + c_{11} xy = c_{00}$ $(1)$}
Ebenengleichung im 3-dimensionalen Raum ist jedoch:
\formel{$ax + by + cz = d$}
(1) lässt sich wegen dem Summanden $c_{11} xy$ nicht in die Ebenengleichung überführen. Daher ist die Fläche im Allgemeinen (d.h. wenn $c_{11} \neq 0$) keine ebene Fläche.
...
$\pnt{p}$ ist differenzierbar und die Tangenten sind linear unabhängig. Damit ist $\pnt{p}$ regulär. Allerdings sind die Tangenten nur in $\pnt{p}(0,0)$ orthonormal. Für alle anderen Punkte gilt:\\\\
\eqs{$|\frac{\partial p}{\partial u}| \neq 1$}
\eqs{$|\frac{\partial p}{\partial v}| \neq 1$}$ $\\\\
Es gibt auch keine Umparametrisierung, die die Tangenten in allen Punkten orthonormal werden lässt.\\

\textbf{b)}

Gegeben sei die folgende reguläre \pnt{q}(u,v):
...
\end{center}
Die Tangenten sind offensichtlich linear unabhängig, aber nicht orthonormal. Eine entsprechende Umparametrisierung sieht wie folgt aus:
...
\eqs{$\frac{\partial q}{\partial v} \eq \threevector{0}{1}{0}$}
\end{center}
Die Parametrisierung erzeugt die gleiche Fläche, allerdings sind die Tangenten orthonormal.

\task{5}

Wird als Basisvektor $(1)$ gewählt, ist die Approximation eine Zur X-Y-Ebene parallele ebene Fläche. Die Normalen entsprechen an allen Stellen:
\formel{$\threevector{0}{0}{1}$}
Offensichtlich ist dies im Allgemeinen nicht die Normale an allen Stellen der approximierten Fläche.
