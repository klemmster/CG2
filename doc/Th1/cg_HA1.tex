...
\\
\textsc{Beweis:} Wir werfen entweder $L3$ (Werte größer als $M$) oder $L1$ (Werte kleiner als $M$) weg. Angenommen wir werfen $L3$ weg. Unter den $n/5$ Werten von $X$ befinden sich $n/10$ gr"oßere Werte als $M$. Wenn ein Wert aus $X$ gr"oßer als $M$ ist, dann sind genau 2 Werte aus $S[i]$ ebenfalls gr"oßer als der Wert aus $X$. Daraus folgt, dass $L3$ mindestens 3 Elemente in jeder der $n/10$ Gruppen von $S[i]$ hat, woraus sich mindestens $3n/10$ Elemente ergeben. Das gleiche gilt f"ur $L1$, wodurch sich h"ochstens $7n/10$ Elemente ergeben ($T(7n/10)$).\\
\\
...
Eine Zelle der Kantenlänge $s$ kann bei einem Punkt-Mindestabstand $\epsilon$ maximal $n_{max} = (\frac{s}{\epsilon} + 1)^2$ Punkte enthalten, da am meisten Punkte in die Zelle passen, wenn die Punkte Gitterförmig angeordnet sind:
...
Folgende Translationen werden für die ersten vier Primitiven durchgeführt ($h$ sei die Höhe des Ausgangsprimitivs):\\
...
In der Kurve entstehen unstetige Sprünge:
{\centering\image{0.1}{images/bspline2.jpg}}

Die Träger der Segmente der Basisfunktionen sind unabhängig vom Grad und hängen nur von den Knots ab.
